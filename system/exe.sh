...
mkdir -p /root/gg/ /mnt/xray /mnt/html /var/log/xray/ /var/www/html/ /root/.acme.sh

# Input domain
read -rp "Input Your Domain For This Server: " SUB_DOMAIN
echo "$SUB_DOMAIN" > /root/gg/domain
...
/root/.acme.sh/acme.sh --installcert -d "$domain" --fullchainpath /root/gg/xray.crt --keypath /root/gg/xray.key --ecc
chmod 600 /root/gg/xray.key /root/gg/xray.crt
